...
export default function ViewCustomerDetails() {
  ...
  const loadRecord = async () => {
    try {
      const response = await axios.get("http://localhost:8090/fingAllCustomer");
  ...
  const searchcustomer = Record.filter((pro) => {
    return (
      pro.customerName &&
      pro.customerName.toLowerCase().includes(search.toLowerCase())
    );
  });
  ...
  return (
    <div className="container mt-5">
        ...
        <div className="col-sm-30 mb-4 mt-2">
          <form onSubmit={(e) => e.preventDefault()}>
            <input
              ...
              type="search"
              role="searchbox"
              placeholder="Enter the Customers Name to search"
              value={search}
      ...
      <div className="py-">
          ...
          <table className="table table-transparent table-striped shadow ">
            ...
            <tbody className="text-center">
              {searchcustomer
                .filter(
                  (st) =>
                    st.customerName.toLowerCase().includes(search) ||
                    st.customerName.toUpperCase().includes(search)
                )
                .map((customer, index) => (
                  <tr>
        ...
        <Link className="btn btn-light m-2" to="/adminHomePage" id="add">
          <svg
